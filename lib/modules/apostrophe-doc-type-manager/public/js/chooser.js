apos.define('apostrophe-doc-type-manager-chooser', {
  ...
  beforeConstruct: function (self, options) {
    ...
    if (self.field.type === 'joinByOne' || self.field.type === 'joinByOneCustom') {
      self.limit = 1;
    } else {
  ...
  construct: function (self, options) {
    ...
    self.set = function (choices) {
      ...
      return self.refresh({
        ignoreCurrentRelationships: true
      });
    };
    ...
    self.add = function (_id) {
      var existing = _.find(self.choices, {
        value: _id
      });
      if (existing) {
    ...
    self.remove = function (_id, refresh) {
      ...
      if (self.allPieceChoises && self.allPieceChoises.length > 0)
        self.$el.closest("form").find('.apos-hidden').toggleClass('apos-hidden')
      if (self.removed) {
        // We are striking things through, not forgetting about them
        var removed = _.find(self.choices, {
          value: _id
        });
        if (removed) {
    ...
    self.refresh = function (options) {
      function hideFields(self) {
        var choice = self.allPieceChoises[0].tipologia
        var managerToShowCriteria = self.options.field.showFields
        var find = _.find(managerToShowCriteria, {
          forType: choice
        })
        _.each(find, function (v, i) {
          if (!$(document).find('[data-name=' + v + ']').hasClass('apos-hidden'))
            $(document).find('[data-name=' + v + ']').toggleClass('apos-hidden');
        })
      };
      options = options || {};
      if (self.refreshing) {
      ...
      self.$choices.html('');
      return self.api('chooser-choices', {
        ...
        self.allPieceChoises = result.allchoices
        ...
        return async.eachSeries(self.choices || [], function (choice, callback) {
          ...
          if (err) {
            // Ouch
            apos.notify('Unable to populate form correctly, try again later', {
              type: 'error',
              dismiss: true
            });
            self.decrementRefreshing();
    ...
    self.enableLinks = function () {
      self.link('apos-delete', 'item', function ($button, _id) {
        var index = _.findIndex(self.choices, {
          value: _id
        });
        if (index === -1) {
          return;
      ...
      self.link('apos-relate', 'item', function ($button, _id) {
        var choice = _.find(self.choices, {
          value: _id
        });
        if (!choice) {
          return;
      ...
      var sortableOptions = {
        ...
        update: function (event, ui) {
          ...
          if ($choices.length !== self.choices.length) {
            apos.notify('Something went wrong, reverting changes', {
              type: 'error'
            });
          ...
          $choices.each(function () {
            self.choices.push({
              value: $(this).attr('data-chooser-choice')
            });
          });
    ...
    self.launchBrowser = function () {
      return self.convertInlineRelationships(function (err) {
        if (err) {
          apos.notify('Please address errors first.', {
            type: 'error'
          });
          return;
        }
    ...
    self.enableInlineSchema = function () {
      self.inlineSchema = _.filter(self.field.relationship || [], {
        inline: true
      });
    };
